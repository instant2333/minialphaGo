...
class boardfunc {
public:
	std::vector<int> BIT = std::vector<int >();
	...
	std::vector<int > gen_movelist(std::array<int, 64> myboard,int curcolor );
	//std::tuple<uint64_t, uint64_t > pop_lsb(uint64_t bitmap);
	bool getwinner(std::array<int, 64> myboard);
	std::array<int, 64> dochange(std::array<int, 64>  myboard, int temppos, int curcolor);
